  ...
  User.findOne({ email: email }) //finding the entered email in the db, if there exist a match throwing a error if not storing it in db
    .then((savedUser) => {
      if (savedUser) {
        return res
          .status(422)
          .json({ error: "User already exists in the fields" });
      }
      bcrypt
        .hash(password, 12) //hashing the password using bcryptjs
        .then((hashedpassword) => {
          const user = new User({
            email,
    ...
    .then((savedUser) => {
      ...
      bcrypt.compare(password, savedUser.password).then((doMatch) => {
        if (doMatch) {
          ...
          res.status(200).send({ token: token }); //using jwt getting a token for the successfully loged user
        } else {
          return res.status(422).json({ error: "Invalid email or password" });
        }
      });
